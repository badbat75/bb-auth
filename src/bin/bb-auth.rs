...
/// The same reasoning is what makes `form-action` `'none'` here and the estate's on the two
/// pages that redirect: each of these carries a link, and a link is not a form submission.
fn respond_card(
    req: Request,
    state: &State,
    status: u16,
