...
| `audit.js` | the Audit tab over the file the gate writes and this GUI never does: newest first, a key event naming the row it acted as, a reason code rendered as a sentence, the three list boxes and the filter box applying together on one submit, the pager, a Refresh that keeps the whole view, and the empty state a deployment that has recorded nothing shows |
